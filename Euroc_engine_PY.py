...
def CEA(Temp_prop,Pressure,eps,O_F):
    ...
    R_spec = R/Mm
    return Pe, ve, Tc, gamma,c_star, R_spec
...
def ChangeTempFixInjector(Temp_):
    ...
    [P_e_,v_e_,T_c_,gamma_,c_star_, R_spec_] = CEA(Temp_,P2_/pow(10,5),eps,m_dyer_dot_/m_fuel_dot_)
...
T = 2460  #Average thrust estimation from rocketpy [N]
...
eps = 3.74                              # Supersonic expansion ratio in the nozzle (Optmial expansion ratio)
print("\033[96mExpansion ratio", eps, "\033[0m")
...
############################## ROUTINE START ###################################
[P_e,v_e,T_c,gamma,c_star,R_spec] = CEA(T_prop,P2/pow(10,5),eps,OF_init)
print("\033[96mExit pressure", P_e, "\033[0m","bar") # Exit pressure [Pa] Check for being close to atmospheric
#print("Specific heat ratio", gamma)
#print("Characteristic velocity", c_star)
#print("Exit velocity", v_e)

#############################################################################################################################
# Finding requried mass flow rate for the given thrust, and the theoretical throat and exit area
m_dot_target = T/v_e
D_t = math.sqrt(((c_star*m_dot_target/P2)/math.pi))*2
D_e = math.sqrt((c_star*m_dot_target/P2 * eps)/math.pi)*2
print("\033[92mThroat data\033[0m")
print("\033[92mTheoretical throat", D_t*1000, "\033[0m","[mm]")
print("\033[92mTheoretical exit area", D_e*1000, "\033[0m","[mm]\n")
mass_fu_target = m_dot_target / (OF_init + 1)
mass_ox_target = mass_fu_target* OF_init
#############################################################################################################################

# ITERATION FOR THE NUMBER OF HOLES OF THE INJECTOR
m_dyer_dot = 0
...
print("\033[93mInjector Data\033[0m")
print("Oxidizer Mass flow rate with", N,"number of holes =", m_dyer_dot, "[kg/s] \n")
#############################################################################################################################
...
print("\033[95mChamber geometry data\033[0m")
...
print("Mass flux verification",G_Ox, "[kg/(s*m^2)] \n")
print("OF ratio verification", m_dyer_dot/m_fuel_dot)
